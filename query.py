...
file_path = "Query_Result/" + day + "/" + time + "/" + "most_product_sold_in_every_province"
f= open(file_path,"w+")
...
file_path = "Query_Result/" + day + "/" + time + "/" + "most_product_sold_in_every_market"
f= open(file_path,"w+")
...
file_path = "Query_Result/" + day + "/" + time + "/" + "total_sales_price_in_every_market"
f= open(file_path,"w+")
...
cur.execute('''SELECT province,SUM(has_sold*price) AS sales_price
				...
				GROUP BY province
				ORDER BY sales_price DESC;''')
file_path = "Query_Result/" + day + "/" + time + "/" + "total_sales_price_in_every_province"
f= open(file_path,"w+")
...
file_path = "Query_Result/" + day + "/" + time + "/" + "total_sales_price_in_every_market_in_last_half_hour"
f= open(file_path,"w+")
f.write("market_id,total_sales_price\n")
rows = cur.fetchall()
for row in rows:
...
cur.execute('''SELECT city,SUM(has_sold*price) AS total_sales_price
				...
				GROUP BY city
				ORDER BY total_sales_price DESC;''')
file_path = "Query_Result/" + day + "/" + time + "/" + "total_sales_price_in_every_city"
f= open(file_path,"w+")
